...
class UPME {
    ...
    function search($args=array()) {
        ...
        $defaults = array(
            ...
            'operator' => 'AND',
            'use_in_sidebar' => null,
            'users_are_called' => __('users','upme'),
            'combined_search_text' => __('Combined Search','upme'),
            'button_text' => __('Filter','upme')
        );
    ...
    function search_result($args) {
        ...
        else
        {
            if($this->upme_args['group_meta'] != '' && $this->upme_args['group_meta_value'] != '')
            {
                $this->search_query_string.=" INNER JOIN " . $wpdb->usermeta . " as mt ON (users.ID = mt.user_id) WHERE 1=1 AND (mt.meta_key = '_upme_search_cache' AND mt.meta_value LIKE '%".$this->upme_args['group_meta']."::" . $this->upme_args['group_meta_value'] . "%')";
                
                $this->count_search_query_string.=" INNER JOIN " . $wpdb->usermeta . " as mt ON (users.ID = mt.user_id) WHERE 1=1 AND (mt.meta_key = '_upme_search_cache' AND mt.meta_value LIKE '%".$this->upme_args['group_meta']."::" . $this->upme_args['group_meta_value'] . "%')";
            }
        }
    ...
    function view_profile($id=null, $width=null, $view=null, $group=null, $show_stats=null, $show_social_bar=null, $use_in_sidebar=null, $users_per_page=null, $hide_until_search=null, $role=null) {
        ...
        if (!$hide_until_search) {
            ...
            if ($users) {
                ...
                $display .= '<p>' . sprintf(__('No %s found matching the selected criteria.', 'upme'), $this->search_args['users_are_called']) . '</p>';
            }
    ...
    function upme_profile_custom_routes() {
        ...
        if (isset($current_option['profile_page_id']) && !isset($_REQUEST['page_id'])) {
            $link = get_permalink($current_option['profile_page_id']);
            $filter_link = rtrim(substr(str_replace(site_url(), '', $link), 1), '/') . '/';
