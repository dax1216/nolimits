...
class Comment_Image {
	 ...
	 *---------------------------------------------*/
	 ...
	 function add_styles() {
		...
		if( is_single() || is_page() ) {
			...
			wp_register_style( 'comment-images',  get_template_directory_uri().'/vendor/comment-images/css/plugin.css' );
	...
	function add_scripts() {
		...
		if( is_single() || is_page() ) {
			...
			wp_register_script( 'comment-images',  get_template_directory_uri().'/vendor/comment-images/js/plugin.min.js', array( 'jquery' ) );
		...
		global $post;
	 	...
	 	if ( 'enabled' == get_option( 'comment_image_toggle_state' ) || 'enable' == get_post_meta( $post_id, 'comment_images_toggle', true ) ) {
		 	...
		 	$html = '<div id="comment-image-wrapper">';
				...
				//$html .= '<input type="file" multiple=multiple name="upload_attachment[]">';
		...
		if( count( $comments ) > 0 ) {
			...
			foreach( $comments as $comment ) {
				...
				if( true == get_comment_meta( $comment->comment_ID, 'comment_image' ) ) {
					...
					$comment->comment_content .= '<footer class="participantsAttachedment">';
					$comment->comment_content .= '<ul class="imgAttached">';
						$comment->comment_content .= '<li><a class="thumbnail" href="' . $comment_image['url'] .'"><img src="' . $comment_image['url'] . '" alt="" /><span class="cover"></span></a></li>';
					$comment->comment_content .= '</ul><div class="clearfix"></div>';
					$comment->comment_content .= '</footer>';
