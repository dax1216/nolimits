	<div class="helpInstruction">
						<div class="helpVideo">
							<img src="<?php echo get_template_directory_uri(); ?>/contents/images/img-videoplayer.jpg" >
						</div>
						<div class="instructionText">
							<h3>INSTRUCTIONS</h3>
							<?php 	$instructions = get_post_meta( $post->ID,'NO_LIMIT_instruction',true);  
								if(!empty($instructions)){
									echo $instructions;
								}
							?>
							<a href="#inline" class="btn-hero fancybox"></a>							
							<div id="inline" style="display:none;width:800px;">
								<div class="popupBody">
									<div class="popTop">
										<div class="hRally">
											<img src="contents/images/pop-heart.png" />
											<div class="hCount">
												<span class="label">Hero Rally:</span>
												<span class="hnumber"><span>#</span>23</span>
											</div>
										</div>
										<div class="hBadge">
											<div class="headerBadge">
												<div class="popBadge"><img class="popBadge" src="<?php echo get_template_directory_uri(); ?>/contents/images/pop-badge.png" /></div>
												<div class="heroText">
													<img src="contents/images/you-are-a-hero.png" />
													<h4>Super Shoe Badge</h4>
												</div>
												<div class="clearfix"></div>
											</div><!--.headerBadge-->
											<div class="hproductDetails">
												<p>This project requires thte purchase of non printable items:</p>
												<table width="100%">
													<tr>
														<th>Name:</th>
														<th>Manufacturer:</th>
														<th>Price:</th>
														<th></th>
													</tr>
													<tr>
														<td>Copper Springs</td>
														<td>Aztec Solutions</td>
														<td class="price">$3.95</td>
														<td><a href="#">Take me there</a></td>
													</tr>
													<tr>
														<td>Soft gel shoe insert</td>
														<td>Shoeis</td>
														<td class="price">$8.42</td>
														<td><a href="#">Take me there</a></td>
													</tr>
													<tr>
														<td>Laces</td>
														<td>Shoel Lace Express</td>
														<td class="price">$2.14</td>
														<td><a href="#">Take me there</a></td>
													</tr>
												</table>
											</div>
										</div>
										<div class="clearfix"></div>
									</div><!--.popTop-->
									<div class="popBtm">
										<div class="artHeading">
											<h3>Thank you for your heroic action helping:</h3>											
										</div>
										<div class="needHelp">
											<div class="helpImg">
												<img src="contents/images/need-help.jpg" />
												<div class="cover"></div>
											</div>
											<div class="helpAddress">
												<h3>Children Need Shoes</h3>
												<p><strong>LOCATION: </strong><br/>
													Kudenga House, 3 Baines <br/>
													Avenue, Corner Prince<br/>
													Edward, St. Marher</p>
												<p>
													<strong>POSTAL ADDRESS:</strong><br/>
													P.O. Box 334, Horace, Zimbabwe</p>
											</div>
											<div class="clearfix"></div>
											<p class="helpCaption">My family and I need shoes! please help!</p>
										</div>
										<div class="helpProduct">
											<a  class="img"  href="contents/images/img-print-1.jpg"><img src="contents/images/img-print-1.jpg" /></a>
											<p align="center"><a href="#" class="nl-btn red large"><img src="images/ico-download.png" /> &nbsp; Download and Print!</a></p>
										</div>
										<div class="clearfix"></div>
									</div><!--popTop-->									
								</div>
							</div>
						</div>
						<div class="clearfix"></div>
					</div>